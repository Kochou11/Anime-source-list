const novelsData = {
           reading: [
             {
               name: "NovelBuddy",
               desc: "Platform for reading light novels, web novels, and translated stories for free. It offers a large, regularly updated library, with a community-driven interface. Intrusive ads are minimal, making for a smooth reading experience.",
               tags: ["Large library", "TTS", "Minimal AD"],
               url: "https://novelbuddy.com/home",
               image: "https://novelbuddy.com/static/common/mangabuddy-icon.png"
             },

    {
      name: "Syosetu",
      desc: "Japanese web novel platform hosting many original light novels where amateur writers publish their stories as they write them. Many popular officially published books started out on this website. (Note: the site is in Japanese use google translate to navigate and read.)",
      tags: ["Web-novels", "Japanese", "Original"],
      url: "https://syosetu.com",
      image: "https://static.syosetu.com/view/images/narou.ico?psawph"
},
{
  name: "NovelsOnline",
  desc: "Free site where you can read and listen to thousands of translated novels. It’s user-friendly, but the intrusive ads can sometimes disrupt your experience.",
  tags: ["ADs", "TTS", "Medium library"],
  url: "https://novelsonline.net/",
  image: "https://scontent.fjkr2-1.fna.fbcdn.net/v/t39.30808-1/241818077_322046539610982_4645327483002923224_n.jpg?stp=cp0_dst-jpg_s50x50_tt6&_nc_cat=109&ccb=1-7&_nc_sid=fe756c&_nc_ohc=uBKvlAMLsnsQ7kNvgEF5ubM&_nc_oc=AdigFOjfGBozsG7l0PrVXAIQNMh37unBzKncX7Ky5p11ZokAAjeS424MBPrEQKgH-e6HMF4nqkPewlRjJEDTQrdj&_nc_zt=24&_nc_ht=scontent.fjkr2-1.fna&edm=ANcbAXIEAAAA&_nc_gid=A0UScLMQCc7ZQzvEvL5fTTY&oh=00_AYCEBUPKQxyLYLMp7cTJftqnNHC5OLTyzNQnVrLkLE_zLQ&oe=67B7CC10"
},
                ],
  // novels apps resources 
  apps: [
    {
      name: 'LNReader',
      url: 'https://github.com/LNReader/lnreader',
      desc: 'Light novel reader app with multiple source support.',
      tags: ['Android', 'Multiple Sources',  'Open source'],
      image: 'https://raw.githubusercontent.com/LNReader/lnreader/master/.github/readme-images/icon_alt.png'
                    },
                    {
                      name: "QuickNovel",
                      url: "https://github.com/LagradOst/QuickNovel",
                      desc: "Android app for downloading novels. It also functions as an Epub reader",
                      tags: ["Android", "Multiple sources", "Open source"],
                      image: "https://www.openapk.net/images/icons/quicknover-android-apk.png"
                    }
                ]
};
